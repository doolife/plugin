...
appModule = (function(appModule, $, undefined){
    ...
    var clickAct = function(){
        ...
        if (controls.target==0) {
            TweenMax.to([$lis.eq(1)], 0.5, {x:$("body").width() / 2, right:0});
            TweenMax.to([$lis.eq(2)], 0.5, {x:$("body").width() / 2, right:0});
        }else if (controls.target==1) {
            TweenMax.to([$lis.eq(0)], 0.5, {x:-$("body").width() / 2, left:0});
            TweenMax.to([$lis.eq(2)], 0.5, {x:-$("body").width() / 2, y:$("body").height() / 2, width:$("body").width(), right:0, bottom:0});
        }else if (controls.target==2) {
            TweenMax.to([$lis.eq(0)], 0.5, {x:-$("body").width() / 2, left:0});
            TweenMax.to([$lis.eq(1)], 0.5, {x:-$("body").width() / 2, y:-$("body").height() / 2, width:$("body").width(), top:0, right:0});
        }
    ...
    var closeAct = function(){
        TweenMax.to([$lis.eq(0)], 0.5, {x:0, width: $("body").width() / 2, left:0});
        TweenMax.to([$lis.eq(1)], 0.5, {x:0, y:0, width:$("body").width() / 2, height:$("body").height() / 2, top:0, right:0, bottom:"50%", left:"50%"});
        TweenMax.to([$lis.eq(2)], 0.5, {x:0, y:0, width:$("body").width() / 2, height:$("body").height() / 2, top:"50%", right:0, bottom:0, left:"50%"});
